...
def on_push_state(*args):
    ...
    # code inserted for BBC radio art: artist == 'BBC Radio' or title == 'BBC Radio 6')
    if 'album' in args[0]:
        album = args[0]['album']
    if 'artist' in args[0]:
    ...
    if 'uri' in args[0]:
        ...
        uri = ''
    print('status:{0}, service:{1}, artist:{2}, uri: {3}'.format(VOLUMIO_DICT['STATUS'],VOLUMIO_DICT['SERVICE'],artist,uri))
    if VOLUMIO_DICT['STATUS'] == 'play' and (VOLUMIO_DICT['SERVICE'] == 'webradio' or VOLUMIO_DICT['SERVICE'] == 'mpd') and (artist == 'BBC Radio' or ((artist == '' or artist == None) and ('bbc' in uri))):
        try:
            with open('/data/plugins/system_hardware/pirateaudio/bbc_radio.json', 'r') as myfile:
                data = myfile.read()
                bbc = json.loads(data)  # parse file
            if bbc['valid_until'] > time():
                args[0]['album'] = bbc['album']
                args[0]['albumart'] = bbc['img_url']
                args[0]['artist'] = bbc['artist']
                args[0]['title'] = bbc['title']
        except:
            print('couldn\'t open radio data file, or process the json')
